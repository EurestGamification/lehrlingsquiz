...
const BreadTypes: React.FC<BreadTypesProps> = ({ onStepFinished }: BreadTypesProps) => {
    ...
    return (
        <div className="breadTypes">
            <p className="breadTypes__heading">
                Sortiere die verschiedenen Brote zu den richtigen Brotsorten.<br />
                Du kannst die Brote einfach anklicken und in den passenden Brotkorb fallen lassen.
            </p>
            <div className="breadTypes__content">
                ...
                <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
                    ...
                    {Object.keys(currentDnDs).map((breadType, bTi) =>
                        <Droppable id={breadType} key={bTi} className={
                            ...
                            {currentDnDs[breadType].map((bread, bi) =>
                                <Draggable id={bread} key={bi} className="breadTypes__content__draggable">{bread}</Draggable>
                            )}
                        </Droppable>
                    )}
                </DndContext>
    ...
    function checkAnswers() {
        ...
        for (var breadType in DnDsSolution) {
            for (var bread of DnDsSolution[breadType]) {
                if (currentDnDs[breadType].indexOf(bread) > -1)
                    correct++;
            }
        }
        ...
        const DnDsCount: number = Object.keys(DnDsSolution).reduce((sum, k) => sum + DnDsSolution[k].length, 0);
